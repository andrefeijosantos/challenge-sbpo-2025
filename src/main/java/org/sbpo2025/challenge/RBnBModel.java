...
public class RBnBModel extends BasicModel {
	
	// Model constraints.
	List<IloRange> constrs;
	
	public RBnBModel(Instance inst) {
		super(inst);
	}
	...
	public void rmvItemAmount(int i, int amt) throws IloException {		
	...
	public void resetItemAmounts() throws IloException {
		int sum;
		for (int i = 0; i < inst.n; i++) {
			IloRange rng = constrs.get(i); sum = 0;
			for(int a : inst.itemsPerAisles.get(i).keySet()) 
				sum += inst.itemsPerAisles.get(i).get(a);
			rng.setBounds(0, sum);
		}
	}
